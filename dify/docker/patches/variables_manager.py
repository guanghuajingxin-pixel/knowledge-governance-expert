    ...
    def convert_rag_pipeline_variable(cls, workflow: Workflow, start_node_id: str) -> list[RagPipelineVariableEntity]:
        ...
        for item in all_second_step_variables:
            if item.get("belong_to_node_id") == start_node_id or item.get("belong_to_node_id") == "shared":
                # 本地补丁（KGE）：流水线变量在 workflows.rag_pipeline_variables 中以
                # default_value 持久化，而 VariableEntity 字段名为 default；
                # 不做映射时 default 恒为 None，导致 Service API 传 {} 运行流水线
                # 报「xxx is required in input form」（配合 base_app_generator.py 补丁生效）。
                if item.get("default") is None and item.get("default_value") is not None:
                    item = {**item, "default": item["default_value"]}
