...
class BaseAppGenerator:
    ...
    def _validate_inputs(
        ...
        *,
        ...
        if value is None:
            # 本地补丁（KGE）：必填变量缺失但配置了默认值时回退默认值，而非直接报错。
            # 知识流水线（RAG Pipeline）的 shared 分段参数（parent_mode/child_delimiter 等）
            # 全部带默认值；Service API 调用方（知识同步引擎）传 {} 即可用默认参数运行，
            # 与官方文档「Pass `{}` if the pipeline has no input variables」的语义保持一致。
            # 仅当必填且无默认值时才维持原有报错。
            if variable_entity.required and variable_entity.default is None:
