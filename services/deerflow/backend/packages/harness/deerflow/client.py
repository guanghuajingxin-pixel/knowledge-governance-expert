...
class DeerFlowClient:
    ...
    def _get_runnable_config(self, thread_id: str, **overrides) -> RunnableConfig:
        ...
        disabled_tools = tuple(sorted(overrides.get("disabled_tools") or []))
        configurable = {
            "thread_id": thread_id,
    ...
    def _ensure_agent(self, config: RunnableConfig):
        ...
        key = (
            ...
            tuple(sorted(cfg.get("disabled_tools") or ())),
        )
        ...
        disabled_tools = tuple(sorted(cfg.get("disabled_tools") or ()))
    ...
    def _get_tools(*, model_name: str | None, subagent_enabled: bool,
        ...
        disabled_tools 中的工具（按 tool.name 匹配）不会绑定给智能体，
        用于运行时按配置停用某个能力（如钉钉检索、澄清提问等）。
        """
        ...
        if disabled_tools:
            ...
            tools = [t for t in tools if getattr(t, "name", "") not in blocked]
        return tools
