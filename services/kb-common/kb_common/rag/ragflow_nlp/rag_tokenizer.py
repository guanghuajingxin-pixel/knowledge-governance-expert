...
# Modified for knowledge-governance-expert: packaged gzip dictionary, lazy init,
# no disk writes, no WordNet/Punkt downloads (Snowball stemming retained).
# PEP 723 metadata
# /// script
...
class RagTokenizer:
    ...
    def _load_dict(self, fnm):
        ...
        try:
            of = gzip.open(fnm, "rt", encoding="utf-8")
            while True:
                line = of.readline()
                if not line:
                    break
            ...
            raise RuntimeError(f"Cannot load tokenizer dictionary: {fnm}")
    ...
    def __init__(self, debug=False, user_dict=None):
        ...
        if user_dict and os.path.exists(user_dict):
            ...
            current_dir_huqie = os.path.join(current_dir, "res/huqie.txt.gz")
            resource_dir_huqie = os.path.join(resource_dir, "res/huqie.txt.gz")
            if os.path.exists(current_dir_huqie):
                ...
                raise FileNotFoundError("Missing packaged huqie dictionary")
            logging.info(f"Using default dictionary: {self.DIR_}")

        self.stemmer = SnowballStemmer("english")
        self.lemmatizer = WordNetLemmatizer()
        self._use_lemmatizer = False  # WordNet only supports English
    ...
    def tokenize(self, line: str) -> str:
        ...
        for L, lang in arr:
            if not lang:
                res.extend([self._normalize_token(t) for t in word_tokenize(L, preserve_line=True)])
                continue
...
def _get():
    ...
    if _instance is None:
        with _init_lock:
            if _instance is None:
                _instance = RagTokenizer()
    return _instance


def tokenize(text):
    return _get().tokenize(text)


def fine_grained_tokenize(text):
    return _get().fine_grained_tokenize(text)
