...
# Modified: isolated imports; local synonyms; numpy cosine; empty queries score zero.
import logging
import json
import re
...
from .expression import MatchTextExpr
...
class FulltextQueryer(QueryBase):
    ...
    def hybrid_similarity(self, avec, bvecs, atks, btkss, tkweight=0.3, vtweight=0.7):
        ...
        a, b = np.asarray(avec, dtype=float), np.asarray(bvecs, dtype=float)
        sims = np.array([(b @ a) / (np.linalg.norm(b, axis=1) * np.linalg.norm(a))])
    ...
    def similarity(self, qtwt, dtwt):
        ...
        for k, v in qtwt.items():
            q += v  # * v
        return (s / q if q else 0.0)  # math.sqrt(3. * (s / q / math.log10( len(dtwt.keys()) + 512 )))
